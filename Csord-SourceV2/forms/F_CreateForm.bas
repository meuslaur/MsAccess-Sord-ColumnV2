        ...
        Begin Section
            ...
            Begin
                ...
                Begin TextBox
                    ...
                    LayoutCachedLeft =8582
                    LayoutCachedTop =9920
                    LayoutCachedWidth =11483
                    LayoutCachedHeight =10265
                    Begin
                        Begin Label
                            OverlapFlags =223
                            TextAlign =3
                            ...
                            LayoutCachedTop =9920
                            LayoutCachedWidth =8521
                            LayoutCachedHeight =10235
                        End
                    End
                End
                Begin TextBox
                    ...
                    Begin
                        Begin Label
                            ...
                            LayoutCachedTop =10374
                            LayoutCachedWidth =8524
                            LayoutCachedHeight =10689
                        End
                    End
                End
                Begin Label
                ...
                Begin TextBox
                    ...
                    LayoutCachedWidth =12646
                    LayoutCachedHeight =9769
                    Begin
                        Begin Label
                            OverlapFlags =223
                            TextAlign =3
                ...
                Begin CommandButton
                    ...
                    LayoutCachedLeft =11530
                    LayoutCachedTop =10430
                    LayoutCachedWidth =11875
                ...
                Begin CommandButton
                    ...
                    LayoutCachedLeft =11530
                    LayoutCachedTop =9976
                    LayoutCachedWidth =11875
                    LayoutCachedHeight =10261
                    UseTheme =0
                    BackColor =14461583
                    BorderWidth =1
                    BorderColor =14461583
...
'//----------------------------------       VAR       ------------------------------
    ...
    Private m_cCreate       As CCreateFormColumn

    Private Const LBL_COLOR As Long = 6710886       '// ForeColor label, 'Texte 1, Plus clair 40%.

    Private m_sCtrPrec      As String               '// Controle précedement sélectionné.
...
Private Sub Form_Load()
    ...
    Set m_cCreate = New CCreateFormColumn
...
Private Sub cmbSelectBdd_Click()
    ...
    RazForm True                        '// Active les controles...
...
Private Sub cmbSelectPicFolder_Click()
    ...
    sPath = OuvreBoite("Dossier des images...", , , , FD_TypeFolderPicker)
    If (sPath = vbNullString) Then Exit Sub

    Me.txtPicFolder = sPath & "\"
...
Private Sub cmbSelectPicAsc_Click()
    ...
    sImg = OuvreBoite("Image...", "*.png,*.jpg,*.bmp", "Sélectionnez l'image ASC", Me.txtPicFolder, FD_TypeFilePicker)
    If (sImg = vbNullString) Then Exit Sub

    '// Récupère que le fichier.
    lTmp = Len(sImg) - InStrRev(sImg, "\")
    sTmp = Right$(sImg, lTmp)

    Me.txtPicAsc = sTmp
    Me.txtPicAsc.SetFocus
    RestaureLabelTxt
...
Private Sub cmbSelectPicDesc_Click()
    ...
    sImg = OuvreBoite("Image...", "*.png,*.jpg,*.bmp", "Sélectionnez l'image DESC", Me.txtPicFolder, FD_TypeFilePicker)
    If (sImg = vbNullString) Then Exit Sub
...
Private Sub RazForm(Optional bActive As Boolean)
    ...
    Me.txtBddSauve.SetFocus
    '// Applique les valeurs par défaul.
    Me.txtBddSauve = vbNullString
    Me.txtFormName = "F_"
    Me.txtClasseName = m_cCreate.OptVarClasse
    Me.txtFunctionName = m_cCreate.OptFunctionName

    Me.txtTbPrefixe = m_cCreate.OptTextBoxPrefix
    Me.txtCmbPrefixe = m_cCreate.OptCmbPrefix
    ...
    '// Active/désactive les contrôle.
    Me.cmdCloseBd.Visible = bActive
    Me.cmbSelectBdd.Visible = Not bActive
    Me.cmbLanceCreation.Enabled = bActive
    If (bActive = False) Then Me.cmbSelectBdd.SetFocus
    Me.lstObjets.Enabled = bActive
    Me.lstFields.Enabled = bActive
    Me.txtFormName.Enabled = bActive
    Me.txtPicFolder.Enabled = bActive
    Me.txtPicAsc.Enabled = bActive
    Me.txtPicDesc.Enabled = bActive
    Me.txtPicFolder.Locked = bActive
    Me.txtPicAsc.Locked = bActive
    Me.txtPicDesc.Locked = bActive
...
Private Function VerifSaisie() As Boolean
    ...
    For Each octr In Me.Controls
        ...
        If (octr.ControlType = acTextBox) Then
            If CtrVisibleEnabled(octr) Then         '// ignore le controle si non visible ou désactivé.
...
' ----------------------------------------------------------------
' Procedure Nom:            CtrVisibleEnabledUnlocked
' Sujet:                    Vérification si control visible, activé.
' Procedure Kind:           Function
' Procedure Access:         Private
' Parameter oCtr (Control): Control a vérifier
' Return Type:              Boolean, TRUE si Visible ou activé.
' Author:                   Laurent
' Date:                     15/04/2022
' ----------------------------------------------------------------
Private Function CtrVisibleEnabled(octr As Control) As Boolean
    'TODO: voir si le control possède une source. Locked
    CtrVisibleEnabled = ((octr.Visible) And (octr.Enabled))
End Function
